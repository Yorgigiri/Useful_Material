...
Проверка осуществляется с помощью расширения [PerfectPixel](https://chrome.google.com/webstore/detail/perfectpixel-by-welldonec/dkaagdgjmgdmbnecmcefdhjekcoceebi?hl=ru).

Допустимы незначительные отличия (в пределах 1-5px), связанные с различием в рендеринге шрифтов.

Допустимы незначительные отличия, связанные с ошибками в макете (различные отступы или размеры у однотипных элементов, погрешности в цветах).

Допустимы отличия, связанные с заменой контента (изображения, текст). 
...
Текст на сайте должен быть типографирован, т.е. прогнан через [Типограф](https://www.artlebedev.ru/typograf/).

Если какой-либо текст намеренно не обрабатывался Типографом, то об этом следует указать либо в комментариях к коду, либо в `README.md`.

## Проверка иконок и изображений

Иконки на сайте должны быть сделаны с помощью SVG, либо PNG + @2x PNG (если иконка имеет сложные растровые эффекты).

Если иконка простая, но SVG отсутствует, либо ее невозможно экспортировать из макета, то следует сообщить об этом менеджеру проекта.

Для контентных изображений должна быть указана @2x-версия. 

Если для каких-либо иконок нет и не будет SVG-версий, то об этом следует указать в `README.md`.

Если для каких-либо контентных изображений нет и не будет @2x-версий, то об этом следует указать в `README.md`.
...
На сайте должна быть страница для проверки используемых шрифтов (кириллица, латиница и прочие используемые символы).
...
На сайте должна быть фавиконки.

При отсутствии фавиконки следует запросить ее у менеджера проекта.

Если клиент намеренно отказался от использования фавиконок на сайте, то это следует указать в `README.md`.

# Функционал

## Проверка шаринга

На сайте должен работать шаринг (при наличии).
...
На всех элементах должен быть плавный ховер.
...
Минимальные размеры до которых стоит проверять адаптивность десктопной версии - 1024px по ширине, и 550px по высоте.
...
4. Пункты 2 и 3 повторяются до тех пор, пока ширина окна не достигнет минимальной - 1024px.
...
Ссылки - интерактивные элементы, при клике на которые происходит переход на другую страницу.
